...
PROJECT_NAME=trece
STACK_NAME=trece-ronyerisson-tangrama-com-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://trece.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t ronyerisson/trece-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t ronyerisson/trece-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name trece-backend-acceptance -d ronyerisson/trece-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name trece-frontend-acceptance --link trece-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ronyerisson/trece-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop trece-frontend-acceptance
	@docker stop trece-backend-acceptance
